...
INSERT INTO ErrorHistory (EventId, ErrorStart, ErrorEnd, PlantId, MachineryId, ErrorNumber, Notes)
...
UPDATE ErrorHistory
SET ErrorEnd = DATEADD(second, ABS(CHECKSUM(NEWID())) % DATEDIFF(second, ErrorStart, GETDATE()), ErrorStart);ErrorStart, ErrorEnd, PlantId, MachineryId, ErrorNumber, Notes)
VALUES (NEWID(), @ErrorStart, @ErrorEnd, 5, 90, 2, 'Pressure KPI was below the expected value (4) and Temperature KPI was >= 9 for more than 1 minute. sensor_57 was changed and KPIs returned to expected values.', '', '');
